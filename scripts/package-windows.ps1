...
$artifact = Join-Path $outputPath "mesh-$cleanVersion-x86_64.msi"
$source = Join-Path $PSScriptRoot "..\packaging\windows\mesh.wxs"

& wix build --nologo -arch x64 -d "MeshExe=$binaryPath" -d "MeshVersion=$cleanVersion" -pdbtype none -o $artifact $source
