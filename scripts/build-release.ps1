...
Copy-Item target\release\hocmesh.exe dist\
Copy-Item target\release\hocmesh-coordinator.exe dist\
Copy-Item target\release\hocmesh-validator.exe dist\
Copy-Item README.md, CODEX_HANDOFF.md, LICENSE, VERSION dist\
Copy-Item docs\*.md dist\docs\
Copy-Item config\*.json dist\config\
$archive = "hocmesh-$((Get-Content VERSION).Trim())-windows-x86_64.zip"
