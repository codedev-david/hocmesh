...
**Layer ranges go through `hocmesh-engine`, which is this repository's own.** It loads blocks `[start, end)` out of a GGUF file and runs a forward pass over an activation handed to it, so a machine that holds a third of a model can do a third of the work. It is CPU-only, it is written for correctness rather than speed, and a model that fits on one machine will run faster through llama.cpp. What it buys is the case where nothing fits. Its forward pass is checked against llama.cpp rather than only against itself: on f32 weights the engine generates exactly what llama.cpp generates, whether it runs the model whole or across three processes that each hold part of it.
...
5. CUDA, ROCm, and Metal execution delegates to an external llama.cpp-compatible process running a *whole* model; native in-process kernels are not bundled, and `runtime-install` fetches a CPU build pinned by digest, which is enough to run inference but not to accelerate it. The native engine does execute a layer range, and since 0.5.1 its output is checked token-for-token against llama.cpp both whole and split -- but it does so on the CPU, at roughly a tenth of llama.cpp's speed, and it implements dense SwiGLU only, so no mixture-of-experts model loads at all. Those three gaps are what stand between the pipeline plan and a model anyone would want to run: `docs/PERFORMANCE.md` has the arithmetic for what distribution can and cannot make fast, and the ordered plan for closing them; `docs/DISTRIBUTED_INFERENCE.md` has the arithmetic for which splits are viable over a network.
