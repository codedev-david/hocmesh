//! Measured evidence that checking MESH work costs less than doing it.
//!
...
use mesh_core::compute::{execute_work, work_cost_mcu};
use mesh_core::reputation::{Reputation, cheating_expected_value, minimum_slash_mcu};
use mesh_core::verify::{self, AUDIT_BUCKETS, AuditNonce, BUCKETS};
...
/// the mesh burned `(V + 2)` times the compute it actually delivered.
...
fn detection_sweep() {
    ...
    println!(
        "shard pays {reward} mCU, slash = {POLICY_SLASH}x that, and the cheater is the most \
         trusted node MESH allows ({:.0}% audit rate)",
        rate * 100.0
    );
    println!(
        "{:>8}{:>7}{:>9}{:>10}{:>10}{:>12}{:>13}",
