...
/// Share of local hardware the operator is willing to lend to the mesh.
///
/// This is a *user consent* record. Nothing in the mesh may cause a node to
...
pub struct ResourceLimits {
    /// Percentage of logical CPUs offered to the mesh (0 = contribute no CPU).
    pub cpu_percent: u8,
    /// Percentage of GPU capacity offered to the mesh (0 = contribute no GPU).
    pub gpu_percent: u8,
...
mod tests {
    ...
    fn scratch_home() -> PathBuf {
        std::env::temp_dir().join(format!("mesh-limits-{}", uuid::Uuid::new_v4()))
    }
