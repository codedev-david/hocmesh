...
#[command(
    name = "mesh-validator",
    version,
    about = "MESH replicated CU ledger validator"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
enum Cmd {
    Serve {
        ...
        db: String,
        #[arg(long, default_value = ".mesh-validator")]
        home: PathBuf,
        #[arg(long)]
        validators: String,
    },
...
async fn serve(listen: &str, db: &str, home: &FsPath, validators: &str) -> Result<()> {
    ...
    tracing::info!(%listen,"MESH validator started");
...
fn validate_reward_membership(store: &LedgerStore, tx: &LedgerTransaction) -> Result<()> {
    if let TransactionEvidence::ProviderReward(e) = &tx.evidence {
        ...
        let parts = mesh_core::compute::split_work(&root_work, shards);
