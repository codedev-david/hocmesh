...
//! arithmetic instead of an O(n^2) probe hocmesh or a question for a server.
//!
...
use hocmesh_protocol::{COORDINATE_DIMENSIONS, NetworkCoordinate};
...
mod tests {
    ...
    fn a_fitted_position_survives_a_restart() {
        let home = std::env::temp_dir().join(format!("hocmesh-px-{}", std::process::id()));
    ...
    fn a_corrupt_or_absent_position_starts_over_instead_of_failing() {
        let home = std::env::temp_dir().join(format!("hocmesh-px-bad-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
