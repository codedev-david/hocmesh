...
use mesh_model::{ChunkRef, ChunkStore, ModelManifest, sha256};
...
mod tests {
    use super::*;
    use mesh_model::{ModelFormat, sha256};
    ...
    async fn http_peer_seeds_and_verifies_all_chunks() {
        let unique = format!("{}-{}", std::process::id(), mesh_protocol_time());
        let source_root = std::env::temp_dir().join(format!("mesh-seed-source-{unique}"));
        let target_root = std::env::temp_dir().join(format!("mesh-seed-target-{unique}"));
