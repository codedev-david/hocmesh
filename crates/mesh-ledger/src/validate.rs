...
fn validate_reserve(tx: &LedgerTransaction, e: &JobReserveEvidence) -> Result<()> {
    ...
    if e.job_id != mesh_protocol::job_id_from_auth(&e.requester_auth) {
...
fn validate_reward(tx: &LedgerTransaction, e: &ProviderRewardEvidence) -> Result<()> {
    ...
    if e.assignment_id != mesh_protocol::assignment_id(&e.job_id, e.shard_index) {
...
pub fn validate_historical_transaction(
    ...
    match (&tx.kind, &tx.evidence) {
        ...
        (TransactionKind::ProviderReward, TransactionEvidence::ProviderReward(e)) => {
            let reward = work_cost_mcu(&e.work);
            if e.reward_mcu != reward
                || e.assignment_id != mesh_protocol::assignment_id(&e.job_id, e.shard_index)
            {
...
pub fn ledger_entry_signing_message(membership_hash: &str, entry_hash: &str) -> String {
    format!("mesh-ledger-v1|{}|{}", membership_hash, entry_hash)
}
...
mod tests {
    ...
    use mesh_protocol::{AuthProof, WorkSpec, canonical_auth_message};
    use std::{
        fs,
        path::PathBuf,
    ...
    fn reserve_from_auth(
        ...
        let job_id = mesh_protocol::job_id_from_auth(&auth);
        let cost: i64 = split_work(work, shards).iter().map(work_cost_mcu).sum();
        LedgerTransaction {
    ...
    fn signed_reward(
        ...
        let result = mesh_core::compute::execute_work(work);
        let reward_mcu = work_cost_mcu(work);
        let assignment_id = mesh_protocol::assignment_id(job_id, shard_index);
    ...
    fn test_dir(name: &str) -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("mesh-ledger-test-{name}-{suffix}"));
