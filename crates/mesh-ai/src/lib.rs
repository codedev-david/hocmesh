...
impl PromptOutput {
    pub fn validate(&self) -> Result<()> {
            ...
            mesh_protocol::hash_bytes(self.text.as_bytes()) == self.output_sha256,
...
pub fn report_inference_body_hash(
    ...
    mesh_protocol::hash_json(&(&request.assignment_id, &request.outputs))
}

pub fn fail_inference_body_hash(
    request: &FailInferenceRequest,
) -> Result<String, serde_json::Error> {
    mesh_protocol::hash_json(&(&request.assignment_id, &request.reason))
}
