...
use hocmesh_protocol::{NetworkCoordinate, ProbeRequest, ProbeResponse};
use std::{
    sync::{Arc, Mutex},
...
mod tests {
    ...
    async fn an_unfitted_peer_is_measurable_but_says_it_is_unsure() {
        ...
        let endpoint = serve(ProbeState::new("hocmesh_peer", tracker)).await;
        let outcome = probe_peer(&reqwest::Client::new(), &endpoint, None, None)
            .await
            .unwrap();
        assert_eq!(outcome.node_id, "hocmesh_peer");
    ...
    async fn a_reported_round_trip_fits_the_responder() {
        let tracker = Arc::new(Mutex::new(Vivaldi::seeded(b"peer")));
        let endpoint = serve(ProbeState::new("hocmesh_peer", tracker.clone())).await;
        ...
        let router = Router::new().route(
            ...
            post(move |Json(_): Json<ProbeRequest>| {
                ...
                async move {
                    ...
                    Json(ProbeResponse {
                        node_id: "hocmesh_responder".to_string(),
                        coordinate: None,
                    })
                }
            }),
    ...
    async fn the_reported_round_trip_is_the_best_of_several_after_a_warm_up() {
        ...
        assert_eq!(
            outcome.node_id, "hocmesh_responder",
