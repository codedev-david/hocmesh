...
| Validator self-repair | Implemented | A certificate applies only over the head it names, so a seat that missed one commit used to refuse every entry after it while still answering balance queries with a stale number signed as current. `catch_up_to` fetches and applies the gap from the rest of the set — verifying each certificate against the set that governed its height — when a certificate lands above the local head and on a 1s heartbeat. `fetch_certificates` no longer takes an empty page from one behind seat as the end of the chain, and `balance_quorum` waits out ordinary commit lag before reporting a split, naming what each camp of seats held |
...
| Scheduling by hardware/network/reliability/locality | Implemented | `poll_work` scores a bounded oldest-first window: benchmark converted to mCU/s through `REFERENCE_OPS_PER_MCU` with a hard refusal past the lease, round trip amortised against shard size, exposure limited by audit standing, affinity to shards and manifests already held, a scarcity term that prefers the smallest machine which fits and ranks a GPU node down for work no GPU helps with, and a starvation bonus that outranks any fit. Reliability folds a Laplace-smoothed acceptance ratio and the node's current audit rate through `standing()`. None of it touches price: validators recompute the reward from `split_work`, so a coordinator paying a premium for scarce hardware would be proposing a settlement the quorum rejects on sight |
...
| Layer-range execution engine | Implemented | `hocmesh-engine`: `Stage::load(&mut file, start..end)` reads only the tensors for blocks `[start, end)`, and `forward` runs RMS norm, RoPE, grouped-query attention and SwiGLU over an activation it was handed. F32, F16, BF16, Q8\_0, Q4\_0, Q4\_1, Q5\_0, Q5\_1; an architecture not on the known list is refused at load rather than guessed, because a wrong RoPE layout generates fluent nonsense instead of failing. Position travels with the activation, so a gap is an error rather than a hole |
| Split execution is exact | Implemented | `split_matches_whole.rs`: the same model cut two, three, four and eight ways gives **bit-identical** logits — every supported weight format, several grouped-query head ratios, activations round-tripped through the wire encoding on each hop. Guarded against vacuity: `inf == inf` and identical NaN patterns compare equal, so a separate test asserts finite logits with a real spread and more than one winning token |
| Distributed inference | Implemented | `stage-serve` puts one layer range behind a port with `--next`; the tail returns logits down the chain. `stage-run` drives a chain or runs the model whole to compare. `distributed_inference.rs` runs three OS processes over three shards each holding ~41% of the file — asserted by reading the shard — producing output identical to the whole model down to the SHA-256 of the logits. Still one host: the multi-host run remains open |
| Partial model materialisation | Implemented | `model-shard` writes a file at the model's full declared length so every tensor sits where the header says, with the rest a hole, and a `.shard.json` sidecar recording which bytes are real — a hole reads back as zeros and zeros are a valid weight matrix. `stage-serve` checks the sidecar before reading a weight and refuses to start with the missing range named |
| Reference parity | Implemented | `reference_parity.rs` drives llama.cpp as a server, fed token ids so no tokeniser sits between the two. On f32 weights the unsplit engine and a three-stage split both generate what llama.cpp generates, token for token. Every quantised format decodes bit-identically to llama.cpp's own decoding. CI installs the runtime and sets `HOCMESH_REQUIRE_REFERENCE=1`, so a missing reference fails rather than skips. The suite runs on a generated fixture because that is what CI can afford; the same comparison was also run by hand on a downloaded SmolLM2-135M checkpoint -- 30 blocks, grouped-query attention, tied output head -- whole and split three ways, matching llama.cpp on every prompt tried |
...
| Pipeline parallelism | Implemented end-to-end on CPU | Complete layer plans, ordered tensor transport, and an engine that actually executes a stage. GPU stage execution still goes through the llama.cpp adapters, which load whole models, so the distributed path and the accelerated path do not yet meet |
| Model/tensor parallelism | Implemented control/data plane | Contiguous ranks and tensor transport; collective kernels require a runtime plugin. Splitting a single matmul across a network is arithmetically unattractive and is not the path this project takes — layer-range pipelining is |
...
| Linux installer | Implemented | Native DEB and RPM, metadata/content validation, extracted peer execution smoke test. Both scripts open the finished package and read back its contents and its relationship fields — `Provides`/`Conflicts`/`Replaces` on the deb, `Conflicts`/`Obsoletes` on the rpm — so a package that cannot replace its desktop counterpart fails the build |
| Desktop installers | Implemented | Built and content-checked on all three platforms in CI, published alongside the headless installers on a tagged release. Windows produces MSI and NSIS, macOS a DMG and `.app`, Linux a DEB, an RPM and an AppImage |
