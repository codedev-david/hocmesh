...
| Reference parity | Not implemented | The forward pass is not compared against llama.cpp on a converted model. The split is proved to change nothing; that the unsplit result matches another implementation is not. The tensor directory *is* cross-checked against a foreign writer; the arithmetic is not |
